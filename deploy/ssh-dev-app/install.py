...
GSTREAMER_IMPORT_SMOKE: Final = (
    ...
    "render_luma_bitmap,validate_native_overlay_dependencies);"
    "Gst.init(None);"
    ...
    "missing=[name for name in ('queue','capsfilter','libcamerasrc','v4l2h264enc',"
    "'alsasrc','audioconvert','audioresample','voaacenc','aacparse','fakesink') "
    "if Gst.ElementFactory.find(name) is None];"
    "__import__('sys').exit('missing GStreamer factories: '+','.join(missing)) "
    "if missing else None;"
    "bitmap=render_luma_bitmap('TIME UNSYNCED\\nGPS INVALID');"
    "assert len(bitmap)==1152*64;"
    "overlay=NativeNv12OverlayCore();"
    "overlay.set_text('TIME UNSYNCED\\nGPS INVALID');"
    "initial=overlay.snapshot();"
    "assert initial.updates==1 and initial.enabled is True;"
    "assert initial.state=='UNCONFIGURED' and initial.mappings_cached==0;"
    "overlay.set_text(None);"
    "silent=overlay.snapshot();"
    "assert silent.enabled is False and silent.frames_seen==0;"
    "overlay.close();"
    "assert overlay.snapshot().mappings_closed==0"
)
