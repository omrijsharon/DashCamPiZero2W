...
RETIRED_UNIT_SOURCES = (Path("systemd/dashcamd.socket"),)
...
def prepare(
    ...
    *,
    ...
    if any((repository / path).exists() for path in RETIRED_UNIT_SOURCES):
        raise ValueError("retired systemd socket activation source is still present")
