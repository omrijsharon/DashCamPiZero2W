...
def _device(*, card: int, serial: str = "mic-serial", path: str = "1-1.2") -> AlsaCaptureDevice:
    return AlsaCaptureDevice(AlsaIdentity("1234", "aBcD", serial, path, "USBMic"), card, 0)


def test_stable_identity_match_uses_current_index_only_after_selection() -> None:
    selector = AlsaSelector("1234", "abcd", serial="mic-serial")
...
def test_same_volatile_card_index_cannot_substitute_a_different_microphone() -> None:
    selector = AlsaSelector("1234", "abcd", serial="wanted")
    impostor = AlsaCaptureDevice(AlsaIdentity("1234", "abcd", "other", "1-1.2"), 1, 0)
...
def test_selector_can_bind_a_serialless_microphone_to_its_configured_usb_path() -> None:
    selector = AlsaSelector("1a2b", "3c4d", physical_path="1-1.4")
    expected = AlsaCaptureDevice(AlsaIdentity("1A2B", "3C4D", physical_path="1-1.4"), 3, 2)
    elsewhere = AlsaCaptureDevice(AlsaIdentity("1a2b", "3c4d", physical_path="1-1.5"), 1, 0)
...
def test_configured_stable_selector_parser_rejects_volatile_alsa_endpoint() -> None:
    selector = parse_alsa_selector("usb:vid=1234,pid=abcd,serial=mic-serial")
    assert selector == AlsaSelector("1234", "abcd", serial="mic-serial")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        lambda: AlsaIdentity("123", "abcd", serial="x"),
        lambda: AlsaSelector("1234", "abcd", serial="\n"),
    ],
)
